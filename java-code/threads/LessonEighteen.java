...
*/				
 ...
 // Adds a Thread to the pool. Tells that thread to 
 //start executing   after 0 seconds (immediately) 
 //and then execute every 2 seconds
 ...
 // In order to fund out we will/ Create an array of threads 
 //with enough spaces for all active threads using (Thread.activeCount)
...
// After this command we will see that there are 4 threads
// First thread  is a thread named 'main'
// And after that the 3 threads that we  have  created
 ...
 for(Thread i : listOfThreads){
			System.out.println(i.getPriority());// 5 5 5 5
	}
